...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val teacherTable = FirebaseDatabase.getInstance().reference
        ...
        btLogin.setOnClickListener {
            ...
            if (edtMkpt.text.toString() == "" || edtPw.text.toString() == "") {
                Toast.makeText(applicationContext, "Enter user id and password", Toast.LENGTH_LONG)
                    .show()
            } else if (edtMkpt.text.length < 4) {
                Toast.makeText(applicationContext, "Enter the correct user id", Toast.LENGTH_LONG)
                    .show()
            } else {

                val userid = edtMkpt.text.toString()
                        ...
                        override fun onDataChange(dataSnapshot: DataSnapshot) {
                            ...
                            if (dataSnapshot.exists()) {
                                ...
                                val decryptedPassword =
                                    teacher!!.password.decrypt("amsucsmapp123456")
                                Log.d("Tyler", "name is: ${teacher!!.name} and password is: $decryptedPassword")
                                if (edtPw.text.toString() == decryptedPassword) {
